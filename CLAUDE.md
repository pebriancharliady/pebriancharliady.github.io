...
- *paper* (`#e9eaf0` + ink text, punch holes, hanko 認 stamp, crimson-multiply photo prints) — works stack, featured grid, works archive, detail-page panels
- *crimson field* (`#7F1324`) — footer; plus *noir black stage* for the services stack
...
**Key components:**
- `components/fx/` — `Decode` (text scramble; renders visible + `.sr-only` copy), `Reveal` (IO; variants up/clip/stamp), `scrollfx` (`getScrollY()` = smoothed-or-native scroll — use it, never raw `pageYOffset`; `ScrollVars` sets `--scroll-y/--scroll-skew/--hero-exit` on `<html>`; `Ticker`, `Parallax`, `ScrollScale`), `smoothScroll` (ScrollSmoother + `[data-pin-smooth]`), `useStack` (the showcase engine: pins a 100vh window and deals cards vertically over each other — incoming card rises/settles with optional `tilt` (works) or bg `parallax` (services, needs the CSS bleed on `.bg`), covered card is pressed back + dimmed via its `.press` span; scrub/snap/`body.reel-active` HUD fade; desktop pins by transform, mobile adds `.is-touch` and pins by sticky, no snap; `.is-stack` gates the pinned CSS, absence = plain vertical fallback), `useSceneHold`, `useEpisode`, `cursorLens`, `analogTv` (breathing warp filter + grain, both desktop-only; NO glitch bursts — they read as flicker and janked phones), `sigil`.
...
- `components/home/` — `hero` (layered name/portrait, MRZ, redaction gag), `worksReel` (paper) + `servicesReel` (noir, `$dark` shell variants) — both are vertical card stacks despite the legacy names; shared shell styles (`ReelWrap/ReelWindow/ReelChrome/StackField`) live in `worksReel/style.js`.
...
**Tuning knobs:** `REEL_SLUGS` (curates the works stack by slug, in dealing order — FILE numbers stay tied to the date-sorted archive), `MAX_FEATURED`, `SERVICES` copy/stacks (`pages/index.js` — featured grid = newest non-reel works, auto-hides when empty); stack feel (`panelScroll`/`snap*`/`tilt`/`parallax` options where `useStack` is called, defaults in `fx/useStack.js`); warp (`WARP_*` in `fx/analogTv.jsx`); voyager waypoints (`TARGETS` in `three/shellVoyager.jsx`); grain (`.crt-grain` block in `common/global/index.js`). Service bg images are work covers, wired via the `svc*` file queries in `pages/index.js`.

**Mobile strategy (≤850px):** native scroll (no smoother — deliberate), sticky-based pins, stacks deal via `.is-touch`; the whole analog layer is OFF (no warp, no grain, no scanlines/vignette, no MRZ backdrop-blur) and `ScrollVars` writes only `--hero-exit` — all deliberate Android-perf choices, don't "restore" them; voyager replaced by hero orbit, no cursor lens. `prefers-reduced-motion` disables all motion everywhere (CSS + JS gates) — keep both sides of every gate when adding effects.
